/* CALENDAR - Code Rendering Studio
   ...
   Slots: .slot .slot.avail .slot.picked .slot.taken .slot.past
*/
...
/* Check if a slot is in the past for a given date */
function isSlotPast(dateStr, slotH, slotM) {
  var now = new Date();
  var parts = dateStr.split('-');
  var slotDate = new Date(+parts[0], +parts[1]-1, +parts[2], slotH, slotM, 0, 0);
  // Add 30 min buffer so current slot is still bookable
...
function buildSlots(gridId, hdId, dateStr, onPick) {
  ...
  SLOT_TIMES.forEach(function(slot){
    ...
    if (isTaken) {
      ...
      cls += ' taken'; // reuse .taken CSS for gray styling
      display += ' (past)';
      clickable = false;
    } else {
      cls += ' avail';
    ...
    h += '<div class="'+cls+'" data-s="'+slot.label+'" data-l="'+lbl+'" '+(clickable?'':'style="cursor:default;opacity:.35"')+'>'+display+'</div>';
  });
...
function showBookForm(formId, slot, date, lbl) {
  ...
  f.innerHTML =
    ...
    '<button class="btn-p" id="bk-cl" style="background:transparent;border:1px solid rgba(255,255,255,.4);color:rgba(255,255,255,.75)">&#10005; Cancel</button>' +
    '</div></div>';
  ...
  if (ok) ok.addEventListener('click', function(){
    ...
    var ph = document.getElementById('bk-ph');
    var tp = document.getElementById('bk-tp');
